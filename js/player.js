class Player {
    constructor(skinID, isLeader = false, startJumping = false) {
        ...
        this.sprites = { run: [], jump: [] };
        this.loadAssets();
    }
    loadAssets() {
        const f = `assets/skin${this.skinID}`;
        for (let i = 1; i <= 4; i++) { let img = new Image(); img.src = `${f}/run${i}.png`; this.sprites.run.push(img); }
        for (let i = 1; i <= 2; i++) { let img = new Image(); img.src = `${f}/jump${i}.png`; this.sprites.jump.push(img); }
    }
    ...
    draw(ctx, inv) {
        let img = (this.state === "jump") ? this.sprites.jump[this.velocityY < 0 ? 0 : 1] : this.sprites.run[this.frame % 4];
        if (!img || !img.complete) img = this.sprites.run[0];
        if (img && img.complete) {
            ctx.save();
            if (this.isDying) { ctx.globalAlpha = 0.5; ctx.filter = "grayscale(1) brightness(2) sepia(1) saturate(5)"; }
            else if (inv) ctx.filter = "drop-shadow(0 0 8px gold) brightness(1.2)";
