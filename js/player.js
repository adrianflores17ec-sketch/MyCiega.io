// =====================================================
// CACHÉ GLOBAL DE SPRITES - se comparte entre todos
// =====================================================
const SPRITE_CACHE = {};
...
// Pre-cargar todas las skins al arrancar la página
...
class Player {
    constructor(skinID, isLeader = false, startJumping = false) {
        ...
        // Referencia al caché — NO crea nuevas imágenes
    ...
    jump() {
        if (!this.isDying && (this.state === "run" || this.state === "onBox")) {
    ...
    updateLeader(groundY) {
        ...
        if (this.state === "jump") {
            this.velocityY += this.gravity * (this.isJumpHeld ? 1.0 : 2.5);
        } else {
            this.velocityY += this.gravity;
        }
        this.y += this.velocityY;
    ...
    updateFollower(data, index) {
        ...
        const targetX = data.x
            - (index * this.personalDistance)
            + this.groupOffsetX
            + Math.sin((Date.now() + this.driftOffset) * 0.003) * 12;
    ...
    draw(ctx, inv) {
        ...
        // Fallback: primer frame de run disponible
        if (!img || !img.complete) {
            img = this.sprites.run.find(i => i && i.complete) || null;
        }
        if (!img || !img.complete) return;

        // IMPORTANTE: resetear filter ANTES de save() para evitar contaminación
        ctx.filter = "none";
        ctx.save();
        ...
        ctx.restore();
        // Forzar reset del filter después de restore (fix para Safari/Chrome móvil)
        ctx.filter = "none";
    }
}
